...
class EmployeeDetail extends Component
{
    ...
    {
        // Use ESS-specific config (hr.employee_ess) when available;
        // fall back to a hardcoded safe set for self-service pages.
        $defaultEssTabs = ['overview', 'personal', 'contact', 'employment'];

        try {
            $essResolver = app(ConfigResolver::class, ['configKey' => 'hr.employee_ess']);
            $essConfig = $essResolver->getConfig();

            $this->selfServiceConfig = [
                'enabled' => $essConfig['self_service']['enabled'] ?? true,
                'allowedTabs' => $essConfig['self_service']['allowed_tabs'] ?? $defaultEssTabs,
                'hideEditButtons' => $essConfig['self_service']['hide_edit_buttons'] ?? true,
                'hideCompensation' => $essConfig['self_service']['hide_compensation'] ?? true,
                'hideBankInfo' => $essConfig['self_service']['hide_bank_info'] ?? true,
                'hideTaxInfo' => $essConfig['self_service']['hide_tax_info'] ?? true,
            ];
        } catch (\InvalidArgumentException $e) {
            // ESS config not found; use safe defaults
            $this->selfServiceConfig = [
                'enabled' => true,
                'allowedTabs' => $defaultEssTabs,
    ...
    public function hideCompensation(): bool
    {
        return $this->isSelfServiceMode && ($this->selfServiceConfig['hideCompensation'] ?? false);
    }

    public function hideBankInfo(): bool
    {
        return $this->isSelfServiceMode && ($this->selfServiceConfig['hideBankInfo'] ?? false);
    }

    public function hideTaxInfo(): bool
    {
        return $this->isSelfServiceMode && ($this->selfServiceConfig['hideTaxInfo'] ?? false);
    }
    ...
    protected function loadData(): void
    {
        ...
        if (!$this->employee) {
            $this->dispatch('showAlert', [
                'type'    => 'error',
                'message' => 'Employee not found or you do not have access to this record.',
            ]);
            abort(404, 'Employee not found or you do not have access to this record.');
            return;
        }
    ...
    protected function loadTabData(string $tab): void
    {
        switch ($tab) {
            ...
            case 'payroll':
                ...
                break;
            case 'payslips':
                $payrollModel = \App\Modules\Payroll\Models\PayrollPayslip::class;
    ...
    protected function loadFieldDefinitions(): void
    {
        ...
        $payrollResolver = app(ConfigResolver::class, ['configKey' => 'payroll.employee_payroll_profile']);
    ...
    public function render()
    {
                ...
                ? [
                    [
                        ...
                        'params' => [
                            ...
                            'params' => ['configKey' => $this->configKey, 'recordId' => $this->recordId, 'inline' => true],
                            'title' => 'Edit Employee',
                        ],
                    ],
                ]
        ...
        return view('hr::livewire.employee-detail', [
