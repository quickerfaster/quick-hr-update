...
class AttendanceEventListener extends DataTableRecordListener
{
    ...
    {
        // Only act on attendance records.
        if (!str_contains($event->model, 'Attendance')) {
            return;
        }

        $oldRecord = $event->oldRecord ?? null;
        $livewireComponent = $event->component ?? null;
